...
void xFlashLoad(void) {
	...
	if (flashExist == 1) {
		...
		uiAdr = FLASH_MUTEX_ADDRESS  - MEM_TEMP_OFF;
		uiFlashGet(uiAdr += 4, (uint32_t*) &modbus_mutex);   // FOR MODBUS MUTEX

		uiAdr = FLASH_TIMEOUT_ADDRESS  - MEM_TEMP_OFF;
		uiFlashGet(uiAdr += 4, (uint32_t*) &timeDelay);  //FOR TIMEOUT
...
void xFlashSave(void) {
	...
	uiFlashSet(uiAdr += 4, (uint8_t*) &modbus_mutex);

	uiAdr = FLASH_TIMEOUT_ADDRESS  - MEM_TEMP_OFF;  // Timeout for Mdtask
	uiFlashSet(uiAdr += 4, (uint8_t*) &timeDelay);
...
uint8_t createJson(char demo[50],char channel[5],char name_reg[10],char name_dev[10], uint16_t val, uint8_t time[6])
{
	memset(demo,0,100);
	strcat(demo,channel);
	strcat(demo,"_");
