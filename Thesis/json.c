...
int main()
{
    ...
    printf("%s",buffer);
    // static uint8_t id_temp = 0, temp1 = 0;
    // uint8_t SUM = sizeof(table1)/sizeof(table1[0]);
    // id_temp = table1[0].deviceID;
    // Device(device,table1[0].deviceID,table1[0].deviceName,table1[0].deviceType);
    // strcat(pub,device);
    
    // for (uint16_t i = 0; i < SUM ; i++)
    // {
    //     if (table1[i].deviceID == id_temp)
    //     {
    //         Attribute(attribute,table1[i].deviceChannel,table1[i].deviceTitle,table1[i].valueType);
    //         strcat(pub,attribute);
    //         strcat(pub,",");
    //     }
    // }
    // strcat(pub,"]");
