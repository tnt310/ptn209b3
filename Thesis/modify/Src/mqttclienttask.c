...
const char *tel_topic = "up/telemetry/60c2ecd762d8c82464c12d63";
const char *pro_topic = "up/provision/60c2ecd762d8c82464c12d63";
const char *test_topic = "up/test/60c2ecd762d8c82464c12d63";
const char *up_command = "up/command/60c2ecd762d8c82464c12d63";
const char *command = "down/command/60c2ecd762d8c82464c12d63";
const char *down_pro = "down/provision/60c2ecd762d8c82464c12d63";
...
static void mqtt_incoming_publish_cb(void *arg, const char *topic,u32_t tot_len) {
	...
	if (strcmp(topic,down_pro) == 0) {
		...
		inpub_id = 1;}
	else if (strcmp(topic,test_topic) == 0) {
			/* All topics starting with 'A' might be handled at the same way */
			inpub_id = 2;}
}
...
void mqtt_connection_cb(mqtt_client_t *client, void *arg,mqtt_connection_status_t status) {
	...
	if (status == MQTT_CONNECT_ACCEPTED) {
		...
		if (err != ERR_OK) {
			printf("\r\mqtt_subscribe provision return: %d\n", err);
		}
		err_p = mqtt_subscribe(client, command, 1,mqtt_sub_request_cb, arg);
		if (err_p != ERR_OK) {
			printf("\r\mqtt_subscribe command return: %d\n", err_p);
		}
	} else {
...
void netmqttTask(void *arg) {
	...
	mqtt_modbus_thread_up(mqtt_client,tel_topic, pro_topic, up_command, test_topic);
	while (1)
		;
}
