...
//	while (1) {
//		while(modbus_mutex){
//			device.channel =(dynamic + count)->channel;
//			device.id = (dynamic + count)->deviceID;
//			device.func = (dynamic + count)->func;
//			device.regAdr = (dynamic + count)->deviceChannel;
////			device.channel = 0;
////			device.id = 2;
////			device.func = 3;
////			device.regAdr = 4;
...
//	}
	while (1) {
		while(modbus_mutex){
			for (uint8_t i = 0;i < SUM_DEV0; i++){
				for (uint8_t j = 0; j < SUM_DEV0; j++){
	...
	if ((usAddress >= REG_HOLDING_START)&& ((uint8_t)usAddress + usNRegs <= REG_HOLDING_START + REG_HOLDING_NREGS)) {
		...
		case MB_REG_READ:
			...
			while (usNRegs > 0)
			{
				...
				iRegIndex++;
				usNRegs--;
				printf("\r\n  data telemetry: %d\t%d ",xQueueMbMqtt.NodeID, xQueueMbMqtt.RegData.i16data);
				printf("\r\n-------------\r\n");
			}
			break;
		}
