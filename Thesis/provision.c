...
uint8_t create_name(char buffer[20],uint8_t channel,uint8_t id, char *title, char *description)
{
    memset(buffer,0,20);
    char temp[5];
    switch(channel)
...
int main()
{
    ...
    {
       create_name(name,table1[i].channel, table1[i].id, table1[i].title, table1[i].description);
